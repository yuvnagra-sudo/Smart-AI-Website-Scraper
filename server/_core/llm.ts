...
export async function invokeLLM(params: InvokeParams, retries = 3): Promise<InvokeResult> {
  ...
  const payload: Record<string, unknown> = {
    model: modelOverride ?? process.env.OPENAI_MODEL ?? "gpt-5.4-mini",
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        const isRetryable = response.status === 412 || response.status === 429 || response.status >= 500;
        
        if (isRetryable && attempt < retries) {
          // For rate limits (412/429), use longer delays to wait for limit reset
          // For server errors (5xx), use shorter exponential backoff
          let delay;
          if (response.status === 412 || response.status === 429) {
            // Rate limit delays: 15s, 30s, 60s (total 105s)
            delay = 15000 * attempt;
            console.log(`[LLM] Rate limit hit (${response.status}), waiting ${delay/1000}s for limit reset (attempt ${attempt}/${retries})`);
          } else {
            // Server error delays: 2s, 4s, 8s
            delay = Math.pow(2, attempt) * 1000;
            console.log(`[LLM] Server error (${response.status}), retrying in ${delay}ms (attempt ${attempt}/${retries})`);
          }
